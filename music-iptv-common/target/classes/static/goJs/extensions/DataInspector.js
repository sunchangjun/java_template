...
Inspector.prototype.inspectObject = function(object) {
  ...
  for (var k in declaredProperties) {
    ...
    if("itemTitle"==k){
    	mainDiv.append(createTab("资源名",defaultValue || ""));
  ...
  if (this.includesOwnProperties) {
    for (var k in data) {
      ...
      if("itemTitle"==k){
    	  html +=createTab("资源名称",data[k]);

      }
      if("resTypeName"==k){
    	  html +=createTab("资源类型",data[k]);
      }
      if("singerName"==k){
    	  html +=createTab("歌手名称",data[k]);
      }
      if("pos"==k){
    	  html +=createTab("起点",data[k]);
      }
      if("size"==k){
    	  html +=createTab("大小",data[k]);
      }
    }
  }
...
function createTab(key,value){
	return  '		    <div class="form-group">'
		   +'   <label class="col-sm-4 control-label" style="margin-top:10px;margin-bottom:0px;text-align:right;">'+key+'</label>'
		   +'  <div class="col-sm-8">'
		   +'      <input type="text" value="'+value+'" disabled class="form-control" style="margin-bottom: 15px;">'
		   +' </div>'
		   +' </div>';
}
