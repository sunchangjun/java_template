...
var Event = function(_event){
    ...
    //var keycode =(window.Event)? _event.which:_event.keyCode;
...
var Q = Query = {
	...
	getNewAddUrl:function(_url) {
		...
		if(-1 == url.indexOf("http://") && -1 == url.indexOf("https://") && -1 == url.indexOf("file:///")) {
            url = paramObj.serverUrl +"/show/"+ url;
		}
		return url;
	},
 ...
 * @param env 环境
 */
function checkAndToPlay(authResult,_backUrl,mediaFromType,mediaFromId,prid,env,wthxpath) {
	...
	if(auth){
        ...
        window.location.href=Q.getNewAddUrl("wthx_play.htm")+"backUrl="+Q.encode(_backUrl)+"&rid="+program.rid
            +"&type="+mediaFromType+"&mediaFromId="+mediaFromId+"&prid="+prid+"&test="+env+"&wthxpath="+Q.encode(wthxpath);
	}
...
function ajaxForAuth(obj,backUrl, _doSucessFuc, _doFaileFuc,wthxpath) {
	ajax({
		url: paramObj.serverUrl + "/authorization?rid=" +obj.rid+"&userId="+getCAcardNum()+"&type="+obj.type
		+"&prid="+paramObj.prid+"&test="+paramObj.test+"&wthxpath="+Q.encode(wthxpath),// 请求的URL,可传参数
		type: "POST",
...
function log(msg){
	var flag = false;
	try {
        paramObj.errormsg =paramObj.errormsg+msg;
        if(paramObj.errormsg&&flag&&paramObj.test){
...
String.prototype.containsName = function (name){
     if(!this||this==""){
         return false;
	 }
 ...
 * @param wthxPath
 * @param backUrl 返回路径
 * @param split 分割符 ? 或者 & 或者空字符串
 * @param pageable 是否添加分页参数
 */
function getCommonQueryParam(split,rid,prid,wthxPath,backUrl, pageable){
   var queryString = split+"rid="+rid+"&prid="+prid+"&wthxpath="+wthxPath+"&test="+paramObj.test;
   if(pageable){
       queryString+="&pageIndex=0&pageSize=10000";
   }
   if(backUrl!=null){
       queryString = queryString+"&backUrl="+backUrl;
   }
    queryString = queryString+"&userId="+ getCAcardNum()+"&platform=web";
   return queryString;
}
...
function initglobalParam(){
    ...
    paramObj.wthxpath = Q.getDecoded("wthxpath");
}
...
/**
 * obj json对象，将需要打印的值以键值对形式封装成的对象
 * @param obj
 */
function consolelog(obj){
    ...
    try{
        var url = paramObj.serverUrl+"/log?url="+encodeURIComponent(location.href);
        if (!Object.keys) {
