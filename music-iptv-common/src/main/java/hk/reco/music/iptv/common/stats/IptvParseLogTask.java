...
public class IptvParseLogTask {
	...
	public void job() throws Exception {
		...
		for(File file : log_dir.listFiles()){
			...
			if(file.isFile() && file.getName().startsWith("20")  && file.getName().endsWith(".log")){
		...
		while(it.hasNext()){
			...
			for(File day_file:day_files){
				...
				while((line=reader.readLine())!=null){
				    ...
				    //判断一下,日志中现在加了很多以前设计外的元素:monitorLog
			...
			if(!CollectionUtils.isEmpty(day_user_detail_map.values())){
