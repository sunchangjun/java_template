...
import hk.reco.music.iptv.common.service.IptvResService;
...
public class ConsoleDataController {
    ...
    public Object submitChildLink(String vids) {
        ...
        if (StringUtils.isNotBlank(vids)) {
            ...
            i = this.iptvConsoleResService.consoleCommitLinkToProd(link_vids);
        }
    ...
    public Object submitAllChild(Long prid) {
        try {
            this.iptvConsoleResService.consoleCommitAllChildLinkToProd(prid);
            return new JsonResult();
        } catch (Exception e) {
    ...
    public Object submitVer(String vids) {
        ...
        if (StringUtils.isNotBlank(vids)) {
            for (String vid : vids.split(",")) {
            	count += this.iptvConsoleResService.consoleCommitNonLinkToProd(Long.parseLong(vid));
            }
        }
        return new JsonResult(true, "成功将&nbsp;<span style='color:red'>" + count + "</span>&nbsp;条数据提交到生产环境", null);
    }

    /**
     * 将当前资源的本身及其link资源全部提交到生产
     * 该功能在[资源管理->专题管理]下,点击[提交]时使用
     * @param rid 专题的rid
     * @param vid 专题最新版本的vid
     * @return
     */
    ...
    public Object submitParentAndLinkVer(Long rid,Long vid) {
        try {
            this.iptvConsoleResService.consoleCommitNonLinkToProd(vid);//先提交专题本身
            this.iptvConsoleResService.consoleCommitAllChildLinkToProd(rid);//再提交专题的所有items
            return new JsonResult();
        }catch (Exception e){
            e.printStackTrace();
            return new JsonResult(e.getMessage());
    ...
    public Object globalOnline(String rids) {
        ...
        if (StringUtils.isNotBlank(rids)) {
            ...
            for (String id : rid) {
            	this.iptvResService.updateResGlobalDisable(Long.parseLong(id), null);
                i++;
            }
        }
    ...
    /**
     * 新增布局
     * 在管理界面->布局管理上点击右键的[新增布局]和[新增栏目]均为进入这里
     * @param ver
     * @param prid 当操作是'新增布局'时,prid为空
     * @return
     */
    @RequestMapping("addLayOut")
    @ResponseBody
    public Object addlayout(IptvResVer ver, Long prid) {
    	try{
    		this.iptvConsoleResService.consoleAddLayoutNode(ver, prid);
    		return new JsonResult(true, ver);
    	}catch(Exception e){
    		e.printStackTrace();
    ...
    public Object contactColumn(IptvResVer link, Long crid, Long prid) {
        ...
        IptvResVer new_link = IptvResVer.createLink(prid, crid, -1);
        new_link.setLayout_regular(cver.getLayout_regular());
        new_link.setLayout_show_child_title(link.getLayout_show_child_title());
        new_link.setLayout_show_title(link.getLayout_show_title());
        new_link.setLayout_line(link.getLayout_line());
        new_link.setMedia_ok(true);
        new_link.setSort(link.getSort());
        this.iptvConsoleResService.consoleCreateLinkResVer(new_link);
        return new JsonResult(true, new_link);
    }
     ...
     * @return
     */
    @RequestMapping("submitChildLinkByType")
    @ResponseBody
    @Transactional
    public Object submitChildLinkByType(String type) {
        try {
            iptvConsoleResService.consoleSubmitChildLinkByType(type);
            Set<String> keys = redisTemplate.keys("*");
            redisTemplate.delete(keys);
            return new JsonResult();
        }catch (Exception e){
