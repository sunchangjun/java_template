...
public abstract class AppDataAction  {
    ...
    public String show(HttpServletRequest request, HttpServletResponse response,Model model,Boolean test,String extra,ExtraInfo info)
        ...
        //查询布局参数
        List<IptvResVer> vers = this.stbService.get_layouts_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null,user.getUserId(), sysenv, initialPos);
        String index = "index";
        if(vers!=null&&vers.size()>0){
            IptvResVer ver = vers.get(info.getLayout_index());
            model.addAttribute("curenntLayout",ver);
            //处理聚焦,及栏目信息
            index = dealWithFocusAndColumnData(request,ver,sysenv,info,model,user.getPlatform(),user.getUserId());
        }
    ...
    private  String   dealWithFocusAndColumnData(HttpServletRequest request,IptvResVer ver,boolean sysenv,
        ...
        Long rid = ver.getRid();
        String path = initialPos+"|"+ver.getRid();
        if(ver.getChild_type().equals(IptvObjectEnum.layout_top)){
            //获取所有榜单分类
            List<IptvResVer> vers = this.stbService.get_top_category_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, 0l, rid, sysenv, path);
            ...
            //获取所有歌单分类
            List<IptvResVer> vers = stbService.get_diss_category_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, 0l, rid, IptvMsicUtils.parseTest(sysenv), path);
            ...
            //获取所有歌手分类
            List<IptvResVer> vers = stbService.get_singer_category_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, 0l, rid, IptvMsicUtils.parseTest(sysenv), path);
            List<IptvResVerForPage> cates = CatesCoverter.coverCates(vers);
            model.addAttribute("cates",cates);
            reindex ="index02";
            // model.addAttribute("defaultLayout",false);
            ...
            //自定义布局
            List<IptvResVer> vers = stbService.get_layout_by_rid_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, 0l, rid, IptvMsicUtils.parseTest(sysenv), path);
    ...
    public List<IptvResVerForPage> caculateColumnData(List<IptvResVer> vers)
    {
        ...
        for (IptvResVer ver : vers) {
            ...
            page.setTop(Integer.valueOf(top));
            if (page.getLayout_regular().booleanValue()) {
                Layout layout = new LayoutFactory(page).getLayout();
                Integer integer = layout.doSetLayoutInfo();
                page.setColumn_height(Integer.valueOf(integer.intValue() + column_height.intValue() + 50));
                top = top + integer.intValue() + column_height.intValue();
                re.add(page);
            } else {
                ...
                if (page.getList() != null) {
                    for (IptvResVer iptvResVer : page.getList()) {
                        ...
                        if (height.intValue() >= tmpheight) {
                            tmpheight = height.intValue();
                            tmp = iptvResVer;
                        }
                        IptvResVerForPage irrgularitem = new IptvResVerForPage(iptvResVer);
                        irrgularitem.setAbsolute_y(Integer.valueOf(Integer.valueOf(start.split(" ")[0]).intValue()));
                        pagelist.add(irrgularitem);
                    }
                    page.setPage_list(pagelist);
                    if (tmp != null) {
                        String end = tmp.getLayout_end();
                        Integer height = Integer.valueOf(end.split(" ")[1]);
                        tmpheight += height.intValue();
                        top = top + tmpheight + column_height.intValue();
                        page.setColumn_height(Integer.valueOf(tmpheight));
                    }
                    re.add(page);
                }
            }
            if (page.getLayout_show_title().booleanValue()) {
                top += show_title_height.intValue();
            }
        }
        return re;
    }
    ...
    private boolean getSystemev(Boolean env,HttpServletRequest request){
        if(env != null){
            ...
            if(re==null){
                request.getSession().setAttribute(Constant.SYSTEM_ENV,false);
                return false;
            }
            return (boolean)re;
        }
    ...
    public RestResponse search(Boolean test,String pinyin,HttpServletRequest request,String platform,
        ...
        result.setCode(0);
        try {
            IptvPageResult mv = this.stbService.get_search_list_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, prid, rid, pinyin.toLowerCase(), "mv", 0, 10000, test, wthxpath);
            IptvPageResult song = this.stbService.get_search_list_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, prid, rid, pinyin.toLowerCase(), "song", 0, 10000, test, wthxpath);
    ...
    public RestResponse hotSearch(Boolean test,HttpServletRequest request,String platform,
        ...
        result.setCode(0);
        try {
            List<IptvResVer> mv = this.stbService.get_search_reco_list_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, prid, rid, "mv", 5, test, wthxpath);
            List<IptvResVer> song = this.stbService.get_search_reco_list_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, prid, rid,"song", 5, test, wthxpath);
    ...
    public RestResponse get_free_media(Boolean test,HttpServletRequest request,String platform,
        ...
        try {
            ...
            IptvPageResult song =  this.stbService.get_free_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, prid, rid,IptvObjectEnum.song,
                    page.getOffset(), page.getSize(), test, wthxpath);
            List<IptvResVer> list= new ArrayList<>();
            List<IptvResVer> mvList = mv.getVers();
            List<IptvResVer> songList = song.getVers();
    ...
    public RestResponse user_collect_list(Boolean test,HttpServletRequest request,String platform,
        ...
        result.setCode(0);
        try {
            List<IptvResVer> vers = this.stbService.get_user_collect_list_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, prid, rid, test, wthxpath);
            for (IptvResVer ver : vers) {
    ...
    public RestResponse get_cate_content_list(Boolean test,HttpServletRequest request,Integer pageIndex
        ...
        try {
            ...
            IptvPageResult result = this.stbService.get_cate_content_list_impl(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), null, userId, prid, rid,null,page, test, wthxpath);
    ...
    @RequestMapping("removeAllCollect")
    @ResponseBody
    public Object  removeAllCollect( Boolean test,HttpServletRequest request,String platform,
                                     String userId,Long prid,Long rid,String mac,Model model,String wthxpath) throws Exception {
        this.stbService.remove_all_media_collect(IptvPlatform.web.name(), NetworkUtils.getIpAddress(request), mac,userId, prid, rid, test, wthxpath);
        RestResponse response = new RestResponse();
        return response;
    }
